    ...
    public void execute(Player player, String... params) {
        ...
        if (params[0].equals("show")) {
            ...
            if (params.length == 1) { //eventcaller start
                ...
                while (iter.hasNext()) {
                    Player p1 = iter.next();
                    PacketSendUtility.sendSys2Message(p1, player.getName(),
                        "[EVENT] Registering for Event has Started! Type .queue to register to the event!");
                }
            ...
            if (params.length == 2) {//eventcaller start {number of player}
                ...
                while (ita.hasNext()) {
                    Player p1 = ita.next();
                    PacketSendUtility.sendSys2Message(p1, player.getName(),
                        "[EVENT] Registering for Event has Started! Type .queue to register to the event! ( '" + countP + "' Slots Available)");
                }
                log.info("[eventcaller-start{limited]] GM : " + player.getName() + " started an limited Slot of [" + countP + "] Event in mapId '"
                    + player.getWorldId() + "'");
            }
            ...
            log.info("[eventcaller-stop{ported}] GM : " + player.getName() + " stopped the queue process and ported total of [" + count
                + "] players  in mapId '" + player.getWorldId() + "'");
            ...
            log.info(
                "[eventcaller-cancel] GM : " + player.getName() + " event has been canceled for some reasons in mapId '" + player.getWorldId() + "'");
            player.setCountPlayers(0);
        }
    }
}
