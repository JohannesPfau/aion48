        ...
        if (owner.getMasterName().equalsIgnoreCase("PVP-SIEGE")) {
            ...
            if (requested) {
                PacketSendUtility.sendPacket(player, new SM_QUESTION_WINDOW(902247, 0, 0,
                    "In Siege, the other faction is your enemy. Do you want to join Siege? <br><br><br>Location: Kaldor"));
            }
            return;
        } else if (owner.getMasterName().equalsIgnoreCase("2v2v2-" + DFFAService.getInstance().worldName)) {
            if (player.getController().isInCombat() || player.isInPkMode() || player.isInArena() || player.isInFFA() || player.isInBH()
                || player.isInPrison() || player.getLifeStats().isAlreadyDead()) {
            ...
            if (requested) {
                PacketSendUtility.sendPacket(player,
                    new SM_QUESTION_WINDOW(902247, 0, 0,
                        "In 2v2v2 event, everyone is your enemy except your team mate. Do you want to join 2v2v2?<br><br><br>Location: "
                            + DFFAService.getInstance().worldName + "<br>Total Player: " + DFFAService.getInstance().getDFFASize()));
            }
            return;
        }
