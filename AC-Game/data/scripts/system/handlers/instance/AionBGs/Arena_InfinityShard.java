...
public class Arena_InfinityShard extends ArenaManager {
            ...
            case 231073: //Hyperion
                for (Player player : instance.getPlayersInside()) {
                    if (player.isOnline()) {
                        ...
                        dropItems.add(
                            DropRegistrationService.getInstance().regDropItem(index++, player.getObjectId(), npcId, ItemId.KINAH.value(), 20000000)); //Kinah
                    }
                }
                break;
        }
        ...
        switch (npc.getObjectTemplate().getTemplateId()) {
            ...
            case 231073:
                ...
                PacketSendUtility.sendAnnounceMessage(
                    String.format("[BinAion] %s's alliance has conquered <INFINITY SHARD> instance.", player.getName()),
    ...
    public boolean onReviveEvent(Player player) {
        if (player.isInArena()) {
            ...
            TeleportService2.teleportTo(player, this.mapId, this.instanceId, (float) 118, (float) 115, (float) 131.8584, (byte) 0,
