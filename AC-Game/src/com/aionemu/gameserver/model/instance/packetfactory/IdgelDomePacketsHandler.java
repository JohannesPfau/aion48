...
public class IdgelDomePacketsHandler {
        ...
        instance.doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player GroupMember) {
                ...
                PacketSendUtility.sendPacket(GroupMember,
                    new SM_INSTANCE_SCORE(3, idr.getRemainingTime(), idr, player.getObjectId(), 0, player.getRace().getRaceId()));
    ...
    public void sendPlayerDiePacket(final Player player) {
        instance.doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player GroupMember) {
                PacketSendUtility.sendPacket(GroupMember,
                    new SM_INSTANCE_SCORE(4, idr.getRemainingTime(), idr, player.getObjectId(), 60, player.getRace().getRaceId()));
                PacketSendUtility.sendPacket(GroupMember,
                    new SM_INSTANCE_SCORE(3, idr.getRemainingTime(), idr, player.getObjectId(), 60, player.getRace().getRaceId()));
                PacketSendUtility.sendPacket(GroupMember, new SM_INSTANCE_SCORE(7, idr.getRemainingTime(), idr, instance.getPlayersInside()));
            }
        });
    }
    ...
    public void sendPlayerRevivedPacket(final Player player) {
        instance.doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player GroupMember) {
                PacketSendUtility.sendPacket(GroupMember,
                    new SM_INSTANCE_SCORE(4, idr.getRemainingTime(), idr, player.getObjectId(), 0, player.getRace().getRaceId()));
            }
        });
    }
    ...
    protected void sp(final int npcId, final float x, final float y, final float z, final byte h, final int entityId, final int time, final int msg,
        final Race race) {
        idgelTask.add(ThreadPoolManager.getInstance().schedule(new Runnable() {
