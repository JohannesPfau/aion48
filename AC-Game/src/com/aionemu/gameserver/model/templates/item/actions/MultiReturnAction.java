...
public class MultiReturnAction extends AbstractItemAction {
    ...
    public void act(final Player player, final Item ScrollItem, final int SelectedMapIndex) {
        ...
        PacketSendUtility.sendPacket(player,
            new SM_ITEM_USAGE_ANIMATION(player.getObjectId(), ScrollItem.getObjectId(), ScrollItem.getItemTemplate().getTemplateId(), 5000, 0, 0));
        ...
        final ItemUseObserver observer = new ItemUseObserver() {
            ...
            public void abort() {
                player.getController().cancelTask(TaskId.ITEM_USE);
                PacketSendUtility.sendPacket(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId(), ScrollItem.getObjectId(),
                    ScrollItem.getItemTemplate().getTemplateId(), 0, 2, 0));
        ...
        player.getController().addTask(TaskId.ITEM_USE, ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                ...
                PacketSendUtility.sendPacket(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId(), ScrollItem.getObjectId(),
                    ScrollItem.getItemTemplate().getTemplateId(), 0, 1, 0));
            }
        }, 5000));
    }
}
