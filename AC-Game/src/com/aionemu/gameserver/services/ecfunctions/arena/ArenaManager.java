...
public class ArenaManager extends ArenaService {
     ...
     * Award Methods
     */
    protected void deathPlayer(Player loser, Creature lastAttacker, WorldMapInstance instance, int round, List<ArenaScore> score,
        List<Player> players, ScheduledFuture<?> RoundTask) {
        if (lastAttacker.getActingCreature() instanceof Player && loser != lastAttacker) {
            ...
            sendSpecMessage("Round " + round,
                String.format("%s: %s - %s: %s", winner.getName(), winnerScore.getKills(), loser.getName(), loserScore.getKills()), instance);
    ...
    protected ScheduledFuture<?> DoReward(List<Player> players, ScheduledFuture<?> RoundTask, List<ArenaScore> score) {
        ...
        PacketSendUtility.sendAnnounceMessage(String.format("[Arena] Congratulation! %s is the winner..", firstWinner.getName()),
            ...
            public void run() {
                if (player.getEnemy().getLifeStats().isAlreadyDead()) {
                    ...
                    PacketSendUtility.sendPacket(player,
                        new SM_ITEM_USAGE_ANIMATION(player.getObjectId(), 0, 0, (int) TimeUnit.SECONDS.toMillis(5), 0, 0));
                    ThreadPoolManager.getInstance().schedule(new Runnable() {
