...
public class SkillAtkDrainInstantEffect extends DamageEffect {
    ...
    public void applyEffect(Effect effect) {
        ...
        if (hp_percent != 0) {
            effect.getEffector().getLifeStats().increaseHp(TYPE.ABSORBED_HP, effect.getReserved1() * hp_percent / 100, effect.getSkillId(),
                LOG.SKILLLATKDRAININSTANT);
        }
        if (mp_percent != 0) {
            effect.getEffector().getLifeStats().increaseMp(TYPE.MP, effect.getReserved1() * mp_percent / 100, effect.getSkillId(),
                LOG.SKILLLATKDRAININSTANT);
        }
    }
